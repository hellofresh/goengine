...
func main() {
	...
	projector, err := manager.NewAggregateProjector(
		config.EventStoreStreamName,
		domain.BankAccountTypeName,
		config.AggregateProjectionAccountReportTable,
...
func run(projector *postgres.AggregateProjector, listener driverSQL.Listener, logger *zap.Logger) {
