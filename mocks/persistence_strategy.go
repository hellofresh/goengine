...
func (_m *PersistenceStrategy) GenerateTableName(streamName goengine_dev.StreamName) (string, error) {
	ret := _m.Called(streamName)

	var r0 string
	if rf, ok := ret.Get(0).(func(goengine_dev.StreamName) string); ok {
		r0 = rf(streamName)
	} else {
		r0 = ret.Get(0).(string)
	}
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(goengine_dev.StreamName) error); ok {
		r1 = rf(streamName)
	} else {
		r1 = ret.Error(1)
	}
